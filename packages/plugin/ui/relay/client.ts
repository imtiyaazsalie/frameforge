...
export class RelayClient {
  ...
  async connect(): Promise<void> {
    ...
    this.update({
      ...
      lastError:
        ...
        `no Frameforge server on :${this.opts.ports.join(', ')} yet — it connects automatically once ` +
   ...
   * fail. In production `ports` is just [DEFAULT_PORT]: frameforge's leader always binds that one
