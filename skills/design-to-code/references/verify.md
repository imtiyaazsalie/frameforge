# Visual verification: close the loop

Loaded by **design-to-code**. Generated code that you never render is unverified. Don't ship it on
...
## No generic verify tool — drive it per project
...
## Render with the project's runtime

Use what the project actually uses — e.g. Vite: `pnpm build`
...
## Screenshot at the design's real viewport

Use Chrome headless via CDP
...
## Compare against the Figma node and fix

Diff the two (eyeball, or pixelmatch/odiff for a %); for each real discrepancy
...
Re-screenshot at desktop _and_ mobile widths; confirm zero horizontal
