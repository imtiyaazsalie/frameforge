...
The cross-cutting rules every build follows, for assembling screens from existing components and for
authoring new assets alike. These mirror codegen's "reference tokens, not literals" rules, pointed
at the canvas.
...
3. **A consistent scale** (only when neither applies — a vague description into an empty file) — pick
   from one scale, never one-off numbers:
...
## Layout: auto-layout for children, absolute for placement
...
- **Use auto-layout whenever children have a structural relationship** (stacked, side-by-side,
  ...
  padding / itemSpacing / alignment. This keeps the design responsive and matches how codegen reads
  it back (`flex` / `grid`).
...
## Text: set the real font, not the fallback
...
## Rich text: one node, styled ranges
